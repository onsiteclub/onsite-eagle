 ...
 *
 * Called by the operator_2 app after login. Auto-provisions a dedicated
 * Twilio SMS number for the operator and wires the webhook to request-ingest.
 *
 * Idempotent: returns the existing active number if one already exists.
 *
 * Request:  POST { area_code?: "613", country?: "CA", site_id?: UUID }
 * Response: 200 { phone_e164: "+1...", reused: boolean }
 */
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const searchUrl =
      ...
      `?AreaCode=${area_code}&SmsEnabled=true&Limit=1`;
    ...
    // --- Purchase with webhook pre-wired to request-ingest ---
    const webhookUrl = `${SUPABASE_URL}/functions/v1/request-ingest`;
    ...
    if (insertErr) {
      ...
      // We have an orphaned Twilio number. Release it.
