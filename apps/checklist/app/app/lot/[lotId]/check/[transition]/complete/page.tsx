...
// Legacy route — all live traffic goes through /app/check/complete.
interface Props {
  params: Promise<{ lotId: string; transition: string }>
}

export default function LegacyCompleteRedirect({ params }: Props) {
  ...
  useEffect(() => {
    params.then(({ lotId, transition }) => {
      ...
      router.replace(`/app/check/complete?${qs.toString()}`)
    })
