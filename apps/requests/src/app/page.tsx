...
function getRoleFromHost(): "worker" | "operator" | null {
...
export default function LoginPage() {
  ...
  const [hostRole, setHostRole] = useState<"worker" | "operator" | null>(null);
  ...
  function enter(role: "worker" | "operator") {
    if (!name.trim()) return;
    ...
    router.push(role === "operator" ? "/operator" : "/request");
  }

  const roleLabel = hostRole === "operator" ? "Operador" : "Trabalhador";
  const RoleIcon = hostRole === "operator" ? Truck : Wrench;
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-card rounded-2xl p-6 shadow-sm border border-border space-y-5">
          ...
          {hostRole ? (
            ...
            /* Generic host — show both options */
